<section class="main">
<section class="left-bar">

    <h3>
        <a href="../part1/1_1-main.php" class="module-choice">
        wartości, czyli o ukrytych motywach naszego dzialania</a>
    </h3>
        <h3><a href="../part2/2_1-main.php" class="module-choice"> docenianie czyli budowanie fundamentu adekwatnego poczucia własnej wartości</a></h3>
        <h3><a href="../part3/3_1-main.php" class="module-choice" style="color: #028388;"> omówienie, czyli świętowanie sukcesow i analizowanie porażek</a></h3>
        <ul>    
        <li><a href="3_1.php" class="button-choice">efekt</a></li>
            <li><a href="3_2.php" class="button-choice">studium przypadku</a></li>
            <li><a href="3_3.php" class="button-choice">refleksja</a></li>
            <li><a href="3_4.php" class="button-choice">wiedza</a></li>
            <li><a href="3_5.php" class="button-choice">praktyka</a></li>
            <li><a href="3_6.php" class="button-choice">Podcast</a></li>
            <li><a href="3_7.php" class="button-choice">Literatura</a></li>
            </ul>
        <h3><a href="../part4/4_1-main.php" class="module-choice"> planowanie, czyli wyznaczanie celów i skuteczne dążenie do
nich</a></h3>
        <h3><a href="../part5/5_1-main.php" class="module-choice"> wytrwałość, czyli życie jako maraton, a nie sprint</a></h3>

</section>
<section class="content content3">
  